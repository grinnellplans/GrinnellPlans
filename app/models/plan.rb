class Plan < ActiveRecord::Base
  ...
  def clean_text
    ...
    logger.debug('self.plan_______ _' + generated_html)
     for love in loves
       item = love.first
       jlw = item.gsub(/\#/, "\/")
       unless checked[item]
         ...
         if account.blank?
           if item.match(/^\d+$/) && SubBoard.find(:first, conditions: { messageid: item })
             ...
             generated_html.gsub!(/\[" . Regexp.escape(item) . "\]/s, "[<a href=\"board_messages.php?messagenum=$item#{item}\" class=\"boardlink\">#{item}</a>]")
           end
           if item =~ /:/
             if item =~ /|/
